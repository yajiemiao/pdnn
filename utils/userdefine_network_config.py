...
class NetworkConfig():
    ...
    def __init__(self):
        ...
        self.a_n_ins = 0
        self.a_hidden_layers_sizes = []
        self.a_n_outs = 0
        self.a_non_updated_layers = []
        self.v_n_ins = 0
        self.v_hidden_layers_sizes = []
        self.v_n_outs = 0
        self.v_non_updated_layers = []
    ...
    def parse_config_dnn(self, arguments, nnet_spec):
        ...
        if arguments.has_key('branch_spec'):
            branch_spec = arguments['branch_spec']
            self.parse_config_avdnn(branch_spec)

    def parse_config_avdnn(self,branch_spec):
        av_nnet_layers = branch_spec.split('/')
        a_nnet_spec = av_nnet_layers[0]
        v_nnet_spec = av_nnet_layers[1]
        a_nnet_layers = a_nnet_spec.split(':')
        self.a_n_ins = int(a_nnet_layers[0])
        self.a_hidden_layers_sizes = [int(a_nnet_layers[i]) for i in range(1, len(a_nnet_layers))]
        self.a_n_outs = int(a_nnet_layers[-1])
        v_nnet_layers = v_nnet_spec.split(':')
        self.v_n_ins = int(v_nnet_layers[0])
        self.v_hidden_layers_sizes = [int(v_nnet_layers[i]) for i in range(1, len(v_nnet_layers))]
