...
class SdAConfig():
    ...
    def __init__(self):
        ...
        # av-network structures
        self.a_n_ins = 0                  # dimension of audio inputs
        self.a_hidden_layers_sizes = []   # number of units in the audio hidden layers
        self.a_n_outs = 0                 # number of audio outputs, not used for RBM
        self.v_n_ins = 0                  # dimension of video inputs
        self.v_hidden_layers_sizes = []   # number of units in the video hidden layers
        self.v_n_outs = 0                 # number of video outputs, not used for RBM
    ...
    def parse_config_common(self, arguments):
        ...
        # parse av-DNN network branch structure
        av_nnet_layers = arguments['av_branch'].split('/')
        a_nnet_spec = av_nnet_layers[0]
        v_nnet_spec = av_nnet_layers[1]
        a_nnet_layers = a_nnet_spec.split(':')
        self.a_n_ins = int(a_nnet_layers[0])
        self.a_hidden_layers_sizes = [int(a_nnet_layers[i]) for i in range(1, len(a_nnet_layers)-1)]
        self.a_n_outs = int(a_nnet_layers[-1])
        v_nnet_layers = v_nnet_spec.split(':')
        self.v_n_ins = int(v_nnet_layers[0])
        self.v_hidden_layers_sizes = [int(v_nnet_layers[i]) for i in range(1, len(v_nnet_layers)-1)]
        self.v_n_outs = int(v_nnet_layers[-1])
        self.n_ins = self.a_n_ins+self.v_n_ins
