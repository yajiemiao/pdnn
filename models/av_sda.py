...
from layers.av_da import av_dA, av_dA_maxout
from layers.da import dA, dA_maxout

class av_SdA(object):

    def __init__(self, numpy_rng, theano_rng=None, cfg = None, input = None):
        ...
        self.branch_hidden_layers_number = len(self.a_hidden_layers_sizes) # assume audio and video layers same
        ...
        for i in xrange(self.main_hidden_layers_number+self.branch_hidden_layers_number):
            ...
            if i == 0:
                ...
                layer_a_input = self.dA_a_layers[i-1].output
                layer_v_input = self.dA_v_layers[i-1].output
            elif i == self.branch_hidden_layers_number:
                a_input_size = self.a_hidden_layers_sizes[i - 1]
                v_input_size = self.v_hidden_layers_sizes[i - 1]
                layer_a_input = self.dA_a_layers[i-1].output
                ...
                layer_main_input = self.dA_main_layers[i_prime-1].output
            ...
            if i < self.branch_hidden_layers_number:
                dA_a_layer = dA(numpy_rng=numpy_rng,
                              theano_rng=theano_rng,
                              input=layer_a_input,
                              n_visible=a_input_size,
                              n_hidden=self.a_hidden_layers_sizes[i],
                              W=self.dA_a_layers[i].W,
                              bhid=self.dA_a_layers[i].b,
                ...
                dA_v_layer = dA(numpy_rng=numpy_rng,
                              theano_rng=theano_rng,
                              input=layer_v_input,
                              n_visible=v_input_size,
                              n_hidden=self.v_hidden_layers_sizes[i],
                              W=self.dA_v_layers[i].W,
                              bhid=self.dA_v_layers[i].b,
                              sparsity = cfg.sparsity,
                              sparsity_weight = cfg.sparsity_weight,
                              hidden_activation = cfg.hidden_activation,
                              reconstruct_activation = reconstruct_activation)
                ...
                i_prime = i - self.branch_hidden_layers_number
                dA_main_layer = av_dA(numpy_rng=numpy_rng,
                              theano_rng=theano_rng,
                              a_input=self.dA_a_layers[-1].output,
                              v_input=self.dA_v_layers[-1].output,
                              a_visible=self.a_hidden_layers_sizes[-1],
                              v_visible=self.v_hidden_layers_sizes[-1],
                              n_hidden=self.main_hidden_layers_sizes[i_prime],
                              a_W=self.dA_a_layers[i_prime].W,
                              v_W=self.dA_v_layers[i_prime].W,
                              a_bhid=self.dA_a_layers[i_prime].b,
                              a_bhid=self.dA_v_layers[i_prime].b,
                              sparsity = cfg.sparsity,
                              sparsity_weight = cfg.sparsity_weight,
                              hidden_activation = cfg.hidden_activation,
                              reconstruct_activation = reconstruct_activation)
                ...
                i_prime = i - self.branch_hidden_layers_number
                dA_main_layer = dA(numpy_rng=numpy_rng,
                              theano_rng=theano_rng,
                              input=layer_main_input,
                              n_visible=main_input_size,
                              n_hidden=self.main_hidden_layers_sizes[i_prime],
                              W=self.dA_main_layers[i_prime].W,
                              bhid=self.dA_main_layers[i_prime].b,
                              sparsity = cfg.sparsity,
                              sparsity_weight = cfg.sparsity_weight,
                              hidden_activation = cfg.hidden_activation,
                              reconstruct_activation = reconstruct_activation)
            if i < self.branch_hidden_layers_number:
                self.dA_a_layers.append(dA_a_layer)
                self.dA_v_layers.append(dA_v_layer)
            else:
                self.dA_main_layers.append(dA_main_layer)
